...
export const demandeInsp = createAsyncThunk("espaceAgent/Admin/Inspection", async (_, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.Agentuser.token;
...
export const demandeSlice = createSlice({
  ...
  reducers: {
    resetDemandes: (state) => {
      (state.demande = []), (state.isError = false), (state.isSuccess = false), (state.isLoading = false), (state.message = "");
    },
  },
